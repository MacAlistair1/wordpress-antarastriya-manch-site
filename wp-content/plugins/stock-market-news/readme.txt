...
Stockdio's Stock Market News contains a plugin and a widget that provide the means to display a list of stock market news, for any given stock exchange and symbol. Over 65 different stock exchanges and a large number of market indices, currencies and commodities are supported.
...
**symbol**: The company's stock symbol (ex. **AAPL**). For a list of available market indices please visit www.stockdio.com/indices.
...
4. Insert the `[stock-market-news]` shortcode into your post content, customizing it with the appropriate parameters. You also have the option to use the Stock Market News widget included when you install the plugin.

5. For ease of use, a Stockdio icon is available in the toolbar of the HTML editor (see screenshots for details).
...
Stockdio supports over 65 different world exchanges. For a list of all exchanges currently supported, please visit [www.stockdio.com/exchanges](http://www.stockdio.com/exchanges). Once you have found in the list the stock exchange you need, you must pass the corresponding Exchange Symbol using the stockExchange parameter.
