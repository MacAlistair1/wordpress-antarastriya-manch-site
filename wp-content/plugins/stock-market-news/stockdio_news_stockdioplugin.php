...
<?php
	...
	public function stockdio_news_board_sanitize( $input )
    {
		...
		if( isset( $input['default_loadDataWhenVisible'] ) )
            $new_input['default_loadDataWhenVisible'] =  $input['default_loadDataWhenVisible'] ;
		...
		if( isset( $input['default_includeImage'] ) )
            $new_input['default_includeImage'] =  $input['default_includeImage'] ;
		
		if( isset( $input['default_includeDescription'] ) )
            $new_input['default_includeDescription'] =  $input['default_includeDescription'] ;
			
		if( isset( $input['default_imageWidth'] ) )
            $new_input['default_imageWidth'] =  $input['default_imageWidth'] ;	
		if( isset( $input['default_imageHeight'] ) )
            $new_input['default_imageHeight'] =  $input['default_imageHeight'] ;					
		
		if( isset( $input['default_maxItems'] ) )
            $new_input['default_maxItems'] =  $input['default_maxItems'] ;	
		
		if( isset( $input['default_includeRelated'] ) )
            $new_input['default_includeRelated'] =  $input['default_includeRelated'] ;	
		
		if( isset( $input['default_filterSources'] ) )
            $new_input['default_filterSources'] =  $input['default_filterSources'] ;	
		
		if( isset( $input['default_ignoreSources'] ) )
            $new_input['default_ignoreSources'] =  $input['default_ignoreSources'] ;	
		
		if( isset( $input['default_ignoreItems'] ) )
            $new_input['default_ignoreItems'] =  $input['default_ignoreItems'] ;	
		
		if( isset( $input['default_maxDescriptionSize'] ) )
            $new_input['default_maxDescriptionSize'] =  $input['default_maxDescriptionSize'] ;	
	...
	public function stockdio_news_board_symbol_callback()
    {
        ...
        printf(
            '<input type="text" id="default_symbol" name="stockdio_news_board_options[default_symbol]" value="%s" />		
	...
	public function stockdio_news_board_exchange_callback()
        ...
        printf(
            '<select name="stockdio_news_board_options[default_exchange]" id="default_exchange">		
...
function stockdio_news_board_func( $atts ) {
	...
	$output = '<iframe id="'.$iframe_id.'" frameBorder="0" class="stockdio_news" scrolling="no" width="'.$width.'" '.$iframeHeight.' '.$src.'="'.$link.'?app-key='.$api_key.'&wp=1&addVolume=false&showUserMenu=false'.$extraSettings.'"></iframe>';  		
